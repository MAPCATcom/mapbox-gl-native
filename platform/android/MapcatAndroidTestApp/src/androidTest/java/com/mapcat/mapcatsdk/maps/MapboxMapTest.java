...
import com.mapcat.mapcatsdk.testapp.utils.TestConstants;
import com.mapcat.mapcatsdk.testapp.utils.ViewUtils;
    ...
    onView(withId(R.id.mapView)).perform(new MapboxMapAction((uiController, view) -> {
      ...
      mapboxMap.moveCamera(CameraUpdateFactory.newLatLng(latLng), new MapboxMap.CancelableCallback() {
        ...
        public void onFinish() {
          ...
          Assert.assertEquals(cameraPositionLatLng.getLatitude(), latLng.getLatitude(), TestConstants.LAT_LNG_DELTA);
          Assert.assertEquals(cameraPositionLatLng.getLongitude(), latLng.getLongitude(), TestConstants.LAT_LNG_DELTA);
        }
      });
    }));
  }
