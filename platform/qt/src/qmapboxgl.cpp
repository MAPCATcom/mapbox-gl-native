...
{
    d_ptr->backend->updateFramebufferSize(framebufferSize);
...
{
    d_ptr->backend->updateFramebufferObject(fbo);
}
    ...
    , threadPool(mbgl::sharedThreadPool())
{
    ...
    backend = std::make_unique<QMapboxGLRendererBackend>(pixelRatio, *fileSourceObj, *threadPool, settings.contextMode());
