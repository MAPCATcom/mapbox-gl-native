...
public:
    ENoPropertyError(const string& message): runtime_error(message) {}
};
...
const vector<set<int>> canReadOtherVectors = {{6, 8, 10, 13}}; //latins
const set<int> latinLanguages = {6, 8, 10, 13};

const int LANGUAGE_CODEPAGE_UNDEFINED = -1;


LanguageConfig::NameRule::NameRule(int rule_, bool canRead_, bool canTransliterate_, bool isLatin_):
    rule(rule_),
    canRead(canRead_),
...
string LanguageConfig::getText(const PropertyMap& featureProperties, float zoom) const {
    const string* nameProperty = NULL;
    try {
        nameProperty = &extractStringProperty(featureProperties, "name");
    } catch (const ENoPropertyError&) {
        return "";
    }
    string dataCountry;
    try {
        dataCountry = extractStringProperty(featureProperties, "country");
    } catch (const ENoPropertyError&) {}
    ...
    if (!dataCountry.empty()) {
        ...
        if (countryLanguageIt != countryToLanguage.end()) {
            ...
            string fieldName = "name_" + countryLanguage;
            try {
                dataNameOriginal = extractStringProperty(featureProperties, fieldName);
            } catch (const ENoPropertyError&) {
...
bool LanguageConfig::canReadOther(int languageCodeA, int languageCodeB) const {
    if (languageCodeA == languageCodeB) {
        return true;
    }
    for (unsigned i=0;i<canReadOtherVectors.size();++i) {
...
string LanguageConfig::getUiName(const PropertyMap& featureProperties, const string& nameProperty, const NameRule& nameRule) const {
    int rule = nameRule.rule;
    try {
        string uiName = extractStringProperty(featureProperties, "name_" + languageCode);
        return uiName;
    } catch (const ENoPropertyError&) {}
    if (nameRule.isLatin) {
        return nameProperty;
    ...
    if (rule == 1 || rule == 2) {
        try {
            string englishName = extractStringProperty(featureProperties, "name_en");
            return englishName;
        } catch (const ENoPropertyError&) {}
...
const string& LanguageConfig::extractStringProperty(const PropertyMap& featureProperties, const string& propertyName) const {
    auto it = featureProperties.find(propertyName);
    if (it == featureProperties.end()) {
        throw ENoPropertyError(string("Could not find property named \"") + propertyName + "\" in");
    }
    if (!it->second.is<string>()) {
        throw ENoPropertyError(string("Property named \"") + propertyName + "\" is not of type string.");
    }
